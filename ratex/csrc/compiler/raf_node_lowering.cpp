...
class RAFNodeLowering : public NodeLowering {
 ...
 private:
  ...
  lazy_tensors::Shape InferMatMulXX(const ir::ops::MatMulXX* node);
};
...
Var RAFNodeLowering::LowerToRAF(const ir::Node* node) {
  switch (node->op().op) {
    ...
    default: {
      ...
      if (node->op() == *ir::ops::raf_matmul_xx) {
        return LowerMatMulXX(ir::NodeCast<ir::ops::MatMulXX>(node, *ir::ops::raf_matmul_xx));
      }
...
Var BuildMatMul(const std::vector<Var>& ops, const ir::ops::MatMul* node) {
  ...
  // Shape for both A matirx & B matrix >2D so convert to 3Dx3D & do batch_matmul
  if (a_size > 2 && b_size > 2) {
    // A >3D lower to 3D using reshape
    if (a_size != 3) {
    ...
    // Since we are utilizing batch_matmul_nt, we transpose last to dimensions. This is necessary
    // for reshape lowering & bringing back original shape
    ...
    // Return original size if reshaped
    if (a_size != 3) {
      a_shape.pop_back();
  ...
  // Atleast 1 matrix is <3, so convert to 2d and do dense
  // A >2D lower to 2D using reshape
  else if (a_size > 2) {
    x = BindSymbol(raf::ir::Call(Op::Get("raf.op.reshape"),
                                 {x, MakeConstant(TupleInt({-1, a_shape[a_size - 1]}))}));
  }

  // A <2D promote to 2D using expand/unsqueeze
  else if (a_size == 1) {
    x = BindSymbol(raf::ir::Call(Op::Get("raf.op.expand_dims"), {x, MakeConstant(Int(0))}));
  }

  // B >2D lower to 2D using reshape. Need to tranpose as we use dense=matmul_nt & so matmul holds
  // after reshape
  if (b_size > 2) {
    auto trans_axes = lazy_tensors::util::Iota<int64_t>(b_size);
    std::swap(trans_axes[b_size - 2], trans_axes[b_size - 1]);
  ...
  // tranpose B since we are using dense
  else if (b_size == 2)
    y = BindSymbol(raf::ir::Call(Op::Get("raf.op.transpose"), {y, MakeConstant(TupleInt({1, 0}))}));
  // B <2D promote to 2D using expand/unsqueeze
  else if (b_size == 1)
  ...
  // Return correct reshape shape & result for b dot
  if (b_size == 1) {
    a_shape.pop_back();
    return BindSymbol(
        raf::ir::Call(Op::Get("raf.op.reshape"), {mm, MakeConstant(TupleInt(a_shape))}));
  }
  // Return correct reshape shape & result for a dot
  else if (a_size == 1) {
    b_shape.pop_back();
    return BindSymbol(
  ...
  // Return correct reshape shape & result if A's shape was modified
  if (a_size > 2) {
    a_shape.pop_back();
    a_shape.push_back(b_shape[b_size - 1]);
    mm =
  ...
  // Return correct reshape shape & result if B's shape was modified
  else if (b_size > 2) {
...
Var BuildMatMulXX(const std::vector<Var>& ops, const ir::ops::MatMulXX* node) {
  LTC_CHECK_EQ(node->operands().size(), 2) << "Unexpected number of operands";
  Var x = ops[0];
  Var y = ops[1];
  Var mm;
  ...
  // TODO support higher demension conversion to lower dim matmul for nt/tn/tt
  LTC_CHECK_LT(a_size, 5) << "Unsupported shape";
  LTC_CHECK_LT(b_size, 5) << "Unsupported shape";
  ...
  if (a_size > 2 && b_size > 2) {
    ...
    if (a_size > 3) {
      ...
      LTC_CHECK_NE(a_size, unit_dim) << "No unit dim for 4D a matrix - unsupported shape";
      x = BindSymbol(raf::ir::Call(Op::Get("raf.op.squeeze"), {x, MakeConstant(Int(unit_dim))}));
    }
    if (b_size > 3) {
      ...
      LTC_CHECK(!missing_unit_dim || !broadcast)
          << "No unit dim for 4D b matrix  or need to broadcast/reshape - unsupported shape";
      unit_dim = b_dim;
      y = BindSymbol(raf::ir::Call(Op::Get("raf.op.squeeze"), {y, MakeConstant(Int(unit_dim))}));
    }

    mm = BindSymbol(raf::ir::Call(Op::Get("raf.op.batch_matmul" + type), {x, y}));

    if (unit_dim != -1)
      mm = BindSymbol(
          raf::ir::Call(Op::Get("raf.op.expand_dims"), {mm, MakeConstant(Int(unit_dim))}));
    ...
    if (a_size > 2) {
      ...
      unit_dim = std::find(a_shape.begin(), a_shape.end(), 1) - a_shape.begin();
      LTC_CHECK_NE(a_size, unit_dim)
          << "No unit dim for 3D a matrix w/ b matrix <3D- unsupported shape";
      x = BindSymbol(raf::ir::Call(Op::Get("raf.op.squeeze"), {x, MakeConstant(Int(unit_dim))}));
    } else if (a_size == 1) {
      x = BindSymbol(raf::ir::Call(Op::Get("raf.op.expand_dims"), {x, MakeConstant(Int(1))}));
    }
    if (b_size > 2) {
      // b is 3D and a is < 3D, try to squeeze b
      unit_dim = std::find(b_shape.begin(), b_shape.end(), 1) - b_shape.begin();
      LTC_CHECK_NE(b_size, unit_dim)
          << "No unit dim for 3D b matrix w/ a matrix <3D- unsupported shape";
      y = BindSymbol(raf::ir::Call(Op::Get("raf.op.squeeze"), {y, MakeConstant(Int(unit_dim))}));
    } else if (b_size == 1) {
      y = BindSymbol(raf::ir::Call(Op::Get("raf.op.expand_dims"), {y, MakeConstant(Int(1))}));
    }

    mm = BindSymbol(raf::ir::Call(Op::Get("raf.op.matmul" + type), {x, y}));

    // squeeze/expand dims as inverse to input modifications
    if (b_size == 1)
      y = BindSymbol(raf::ir::Call(Op::Get("raf.op.squeeze"), {y, MakeConstant(Int(1))}));
    if (a_size == 1)
      x = BindSymbol(raf::ir::Call(Op::Get("raf.op.squeeze"), {x, MakeConstant(Int(0))}));
    if (a_size > 2)
      x = BindSymbol(
          raf::ir::Call(Op::Get("raf.op.expand_dims"), {x, MakeConstant(Int(unit_dim))}));
    else if (b_size > 2)
      y = BindSymbol(
          raf::ir::Call(Op::Get("raf.op.expand_dims"), {y, MakeConstant(Int(unit_dim))}));
  }

  return mm;
}

Var RAFNodeLowering::LowerMatMulXX(const ir::ops::MatMulXX* node) {
  Var x = loctx()->GetOutputOp(node->operand(0));
  Var y = loctx()->GetOutputOp(node->operand(1));
  return BuildMatMulXX({x, y}, node);
}
...
lazy_tensors::Shape RAFNodeLowering::Infer(const ir::Node* node) {
  ...
  switch (kind.op) {
    ...
    default: {
      ...
      if (kind == *ir::ops::raf_matmul_xx) {
        return InferMatMulXX(ir::NodeCast<ir::ops::MatMulXX>(node, *ir::ops::raf_matmul_xx));
      }
...
lazy_tensors::Shape RAFNodeLowering::InferMatMulXX(const ir::ops::MatMulXX* node) {
  std::vector<Var> ops;
  for (const auto& x : node->operands()) {
    ops.push_back(MakeVar("operand", ToRAFType(x.shape())));
  }
  Var out = BuildMatMulXX(ops, node);
  Expr body = InferType(ExtractBinding(out, ops));
  return ToLTCShape(body->checked_type());
}
