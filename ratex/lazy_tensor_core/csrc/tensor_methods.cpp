...
LazyTensor LazyTensor::matmul_xx(const LazyTensor& input, const LazyTensor& other,
                                 std::string type) {
  std::vector<int64_t> a_shape =
      lazy_tensors::util::ToVector<int64_t>(input.shape().get().dimensions());
  std::vector<int64_t> b_shape =
      lazy_tensors::util::ToVector<int64_t>(other.shape().get().dimensions());
  return input.CreateFrom(ir::MakeNode<ir::ops::MatMulXX>(input.GetIrValue(), other.GetIrValue(),
                                                          a_shape, b_shape, type));
}
